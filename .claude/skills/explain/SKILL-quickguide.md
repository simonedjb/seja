...
- After `drift`: `/plan` to address the drift, `/design` if intent has evolved, or `/pending` to address pending actions surfaced by the check.
- After `drift --promote`: `--apply-markers plan-<id>` to flip STATUS markers once you have written the Decision entries.
