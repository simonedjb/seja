...
63 scripts in `.claude/skills/scripts/` -- validation checks, index generators, conversion utilities, the central `project_config.py` module that parses `project/conventions.md` to supply all scripts with project-specific paths and settings, `run_all_checks.py` (CI-independent validation orchestrator that discovers and runs all check scripts with unified exit codes and JUnit XML output), `check_docs.py` (plugin-based documentation consistency checker with 6 scanners: framework integrity, path liveness, env vars, command refs, terminology, structural-completeness), `apply_marker.py` (sole write path for Human (markers) files, supporting STATUS, ESTABLISHED, INCORPORATED, CHANGELOG_APPEND, and DECISION_APPEND marker types; DECISION_APPEND appends new `### D-NNN:` ADR-shaped decision entries to the `## Decisions` section with auto-assigned IDs), `generate_decision_digest.py` (generates `_output/decision-digest.jsonl` from D-NNN entries in `product-design-as-intended.md` and HIGH/MEDIUM recommendations in advisory logs; invoked conditionally by post-skill step 7d), `backfill_decision_digest.py` (one-time migration utility to extract decisions from existing advisory logs into the decision digest; idempotent with `--dry-run` support), and `generate_changelog_data.py` (extracts structured change data from git history for framework changelog generation; used by `/document --type changelog --framework`).
...
The SEJA framework maintains a public distribution (`seja-public/`, a git subtree) synced from `seja-priv` via `tools/sync_to_public.py`. Some features and scripts are exclusive to the framework development repo and must not appear in the public copy.

### Partial content: conditional markers

For private-only content embedded within shared files (e.g., a `--framework` flag in a SKILL.md that also has public flags), use HTML comment markers:

```markdown
<!-- priv-only-start -->
| `--framework` | No | Framework-exclusive flag description |
<!-- priv-only-end -->
```

The sync tool's `strip_private_sections()` function removes all content between markers (inclusive) when copying `.md` files to `seja-public/`. Markers are invisible in rendered Markdown and self-documenting in source.

### Whole scripts: `scripts/priv/` directory

For scripts that are entirely private (no public use case), place them in `.claude/skills/scripts/priv/`. This directory is excluded from sync (`EXCLUDE_DIRS` in `sync_to_public.py`) and from `collect_source_files()` in `upgrade_framework.py`, so seeded/upgraded projects never receive these scripts.

### Validation

`check_no_private_leaks.py` scans `seja-public/` for remaining markers, known private files, and private directories. Registered in `run_preflight_fast.py` and runs as part of `/check validate`.
