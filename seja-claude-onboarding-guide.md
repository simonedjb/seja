...
Before anything else, you need the [**SEJA Claude Quickstart Kit**](quickstart-kits/seja-claude-quickstart-kit.zip) from the [quickstart kits folder](quickstart-kits/) -- a zip file containing all the portable skills, references, templates, agents, and validation scripts you'll need.

### 0.1 What's in the Zip?

The file `seja-claude-quickstart-kit.zip` contains approximately 130 files organized in the `.claude/` and `.agent-resources/` directory structure:
...
Download the [SEJA Claude Quickstart Kit](quickstart-kits/seja-claude-quickstart-kit.zip) from the [quickstart kits folder](quickstart-kits/).
...
2. Extract the zip into your project root:
   ```bash
   # The zip preserves the .claude/ directory structure
   unzip seja-claude-quickstart-kit.zip -d .
   ```
...
**Important:** The zip does NOT contain project-specific files (`project-*.md`, `CLAUDE.md`, `.claude/settings.json`). These are generated by the `/quickstart` skill in Part 3, tailored to your specific project. The zip DOES include generic rule and agent definitions that serve as starting points.
...
| `/quickstart` | Bootstraps a new project from templates or a spec file; `--package` to export kit; `--upgrade` to update framework | Starting a brand-new project, packaging for reuse |
...
2. Extract the starter kit zip (see Part 0) into the project root:

   ```bash
   unzip seja-claude-quickstart-kit.zip -d .
   ```

   This creates the `.claude/` directory with all skills, references, templates, agents, and scripts already in place.
...
| `/quickstart --package` | Export portable skill system as a zip |
