...
1. Run these checks and collect results:
   ...
   |---|---|---|---|
   | 1 | Skill System Integrity | `python .claude/skills/scripts/critique_skill_system.py`; parse errors/warnings | Total skills, errors, warnings |
   ...
   | 4 | Reference File Completeness | Scan all SKILL.md `metadata.references`; verify each referenced file exists in `product-design/` | Missing references; distinguish `general/` (must exist) from `product-design/` (may not exist in harness-only repos) |
   | 5 | Conventions Completeness | `python .claude/skills/scripts/critique_conventions.py` | PASS if no errors; WARN if only unused definitions; FAIL if any referenced variables undefined |
   | 7 | Constitution Presence | Check `product-design/constitution.md` | PASS if present; WARN if missing with message: "No project constitution found. Run `/design` to generate `product-design/constitution.md` with your project's immutable principles." |
   | 8 | Skill Spec Compliance | `python .claude/skills/scripts/critique_skill_spec.py` | PASS on exit 0; FAIL on exit 1 with violation details |
   ...
   | 10 | Harness Drift | If `source` input provided: `python .claude/skills/scripts/critique_harness_drift.py --source <source> --target <target_root> --plan-output <CRITIQUE_LOGS_DIR>/drift-plan-<id>.md`; parse output. If `source` not provided: skip. | When `source` provided -- PASS if exit 0 (no drift); FAIL if exit 1 with details: "N add, M remove, K revise -- remediation plan: `<path>`"; WARN on exit 2 with stderr. When `source` not provided -- INFO: "Drift check requires `--source <harness-path>`. Run `/critique health --source <path>` to compare against the canonical harness." |
