...
**Next step**: `/design` after a fresh install to define design intent (entities, permissions, metacommunication, personas, standards). If you are new to SEJA, read `general/getting-started.md` first. After `--upgrade`: `/critique health` to verify harness integrity, then `/explain drift` to catch changed conventions.
