...
3. Reserve sequential IDs for all specs upfront by running `python .claude/skills/scripts/reserve_id.py --type onboarding --title '<name-or-role-slug>'` once per spec.
...
6. Launch one Agent per spec **in parallel** (all in a single message with multiple Agent tool calls). Each agent receives:
