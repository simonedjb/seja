...
3. Verify the structure:
   ```
   my-new-project/
   ├── CLAUDE.md              <- generated by /design
   ├── _references/
   │   ├── general/                 <- 20 reusable standards
   │   │   ├── review-perspectives/ <- 16 per-perspective review files
   │   │   ├── onboarding/          <- 8 role/level onboarding profiles
   │   │   └── communication/       <- 5 audience segment profiles
   │   ├── template/                <- 50 templates for /design
   │   └── project/                 <- generated by /design (not in the kit)
   └── .claude/
...
**Important:** The repository does NOT contain project-specific files (`project/*.md`, `CLAUDE.md`, `.claude/settings.json`). These are generated by `/design` in Part 3, tailored to your specific project. The repository DOES include generic rule and agent definitions that serve as starting points.
...
The questionnaire has 12 sections (M plus 0-10). **Section M (metacomm-message)** is an optional first section where you write a metacommunication message -- an agent will extract defaults from it for subsequent questions. **Section 0 (quick-start)** has 10 questions and is enough for a working skeleton:
...
- **DX perspective:** Answer Section quick-start first -- you get a working setup immediately. Come back to Sections 1-10 (see slug table in questionnaire) later as you refine.
- **DESIGN perspective:** If you are a product designer, focus on Sections 0, 2 (domain model and metacomm), 8 (UX patterns), and 9 (visual design) -- the T1 tier. For technical questions in Sections 1, 3, 4, type `?` to trigger a guided discussion or accept the **Recommended** defaults. Sections 5-7 will be auto-generated from your stack choices.
...
**Pitfall to avoid:** Don't skip Section conceptual-design (Section 2). Your conceptual design drives all future planning and code review. A vague conceptual design leads to vague plans.
...
| `project/user-research-new.md` | Does it capture the user research context for this product? |
| `project/journey-maps.md` | Do the user journeys reflect the key flows you want to support? |
...
2. **Invest 15 minutes in the questionnaire** -- especially Section 0 (quick-start) and Section conceptual-design (Section 2); these drive everything else
