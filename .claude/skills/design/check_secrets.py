#!/usr/bin/env python3
# designer: Before you push, I sweep your staged changes (or the whole tree
#   on request) for things that look like API keys, passwords, tokens,
#   connection strings, or AWS credentials, and refuse to let the commit
#   proceed if anything shaped like a secret slipped in. You get a short
#   list of hits with file and line, not a pile of regex hex to decode.
"""
...
Usage
-----
    python .claude/skills/design/check_secrets.py           # scan staged files
    python .claude/skills/design/check_secrets.py --all      # scan all tracked files
    python .claude/skills/design/check_secrets.py --self-test # run built-in validation
