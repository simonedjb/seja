...
argument-hint: "<architecture|behavior|behavior-evolution|code|data-model|drift> [brief]"
...
If the explanation type (architecture, behavior, behavior-evolution, code, data-model, or drift) is not specified or cannot be inferred from the brief, use the AskUserQuestion tool (fallback: numbered text list) to ask which kind of explanation they want:
...
- "6. Drift -- drift between as-coded and as-intended design specs, then optionally sync"
...
| drift | `${RESEARCH_DIR}` | `research` | `research-<id>-<slug>.md` | `Research` | `all` (default), `conceptual-design`, `metacomm`, `--promote`, `--promote --apply-markers plan-NNNNNN` | dispatched to `.claude/skills/_internal/explain/drift/SKILL.md` |
...
If mode is `drift`: load design-spec references on demand -- read and inject `product-design/product-design-as-intended.md` and `product-design/product-design-as-coded.md` (skip silently if absent -- workspace mode).

Read `.claude/skills/_internal/explain/drift/SKILL.md` via the Read tool and execute its instructions inline as part of this skill's flow.
