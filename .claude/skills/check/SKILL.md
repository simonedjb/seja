...
1. Run the following checks and collect results:
   ...
   Scan all `.claude/skills/*/SKILL.md` files for `metadata.references` entries. For each referenced file, verify it exists in `.agent-resources/`. Report any missing references. Distinguish between `general-*` references (should always exist) and `project-*` references (may not exist in framework-only repos).
