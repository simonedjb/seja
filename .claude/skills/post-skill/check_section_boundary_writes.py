#!/usr/bin/env python3
# designer: When a skill writes into one of the multi-section as-coded files,
#   I check that each edit stays inside a single H2 domain section and
#   refuse the commit if any one change run would span two sections at
#   once. This is how the Conceptual Design, Metacommunication, and Journey
#   Maps regions of your project's as-coded file stay cleanly separated
#   instead of drifting into one another.
"""
...
Usage
-----
    python .claude/skills/post-skill/check_section_boundary_writes.py --staged
    python .claude/skills/post-skill/check_section_boundary_writes.py --range HEAD~1..HEAD
    # Testing only (requires SEJA_ALLOW_TEST_DIFF_INPUT=1):
    SEJA_ALLOW_TEST_DIFF_INPUT=1 python .claude/skills/post-skill/check_section_boundary_writes.py \\
        --diff-from-file path/to/synthetic.diff

Exit codes: 0 pass (or no-op empty registry), 1 violation, 2 runtime error.
"""

# Rationale for design choices and historical context: see check_section_boundary_writes-rationale.md in this directory.
