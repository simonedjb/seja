...
1. Run these checks and collect results:
   ...
   | 10 | Harness Drift | If `source` input provided: `python .claude/skills/scripts/critique_harness_drift.py --source <source> --target <target_root> --plan-output <CHECK_LOGS_DIR>/drift-plan-<id>.md`; parse output. If `source` not provided: skip. | When `source` provided -- PASS if exit 0 (no drift); FAIL if exit 1 with details: "N add, M remove, K revise -- remediation plan: `<path>`"; WARN on exit 2 with stderr. When `source` not provided -- INFO: "Drift check requires `--source <harness-path>`. Run `/critique health --source <path>` to compare against the canonical harness." |
