...
Downstream: `/critique`, `/explain`, `/seja-setup`, `/research`, and `/implement` all apply the same phrasing when presenting decision-point options. Consistency across skills is what makes the convention recognizable -- a user who has seen the pattern once in /plan knows what it means in /critique's interactive modes.
