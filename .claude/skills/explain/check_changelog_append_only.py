...
#   file, I look at the sections that are meant to grow only at the
#   end -- changelogs and append-only journals -- and stop the commit
#   if any prior line was reordered, rewritten, or removed. You keep
#   the historical record intact without thinking about it.
"""
...
discipline, not git-history integrity, and (c) the harness's broader threat
model (see .claude/references/general/threat-model.md) treats history rewrites as a
separate trust boundary.

Usage
-----
    python .claude/skills/explain/check_changelog_append_only.py --staged
    python .claude/skills/explain/check_changelog_append_only.py --range HEAD~1..HEAD
    # Testing only (requires SEJA_ALLOW_TEST_DIFF_INPUT=1):
    SEJA_ALLOW_TEST_DIFF_INPUT=1 python .claude/skills/explain/check_changelog_append_only.py \\
        --diff-from-file path/to/synthetic.diff

Exit codes: 0 pass (or no-op empty registry), 1 violation, 2 runtime error.
"""

# Rationale for design choices and historical context: see check_changelog_append_only-rationale.md in this directory.
...
# human_markers_registry.py lives in the sibling scripts/ directory
