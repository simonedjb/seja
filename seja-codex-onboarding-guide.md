...
The **SEJA Codex Quickstart Kit** is the portable starter kit for a new Codex-powered project. Download it from [quickstart-kits/seja-codex-quickstart-kit.zip](quickstart-kits/seja-codex-quickstart-kit.zip), unzip it into a new repository, and then let Codex customize it for your project.
...
The current Codex kit packages **145 project-independent files**:
...
| `$package-quickstart` | `$quickstart --package` | Packaging now lives inside quickstart |
...
2. Unzip the kit into the repository root.
...
From a new repository root, extract the kit so `.codex/` and `.agent-resources/` land directly in the project root.

```bash
unzip seja-codex-quickstart-kit.zip -d .
```
