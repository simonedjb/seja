#!/usr/bin/env python3
# designer: When you want to see the skill lifecycle as a picture rather than
#   a table -- which skill hands off to which, colour-coded by category -- I'm
#   the generator that reads `skill-graph.md` and writes a Mermaid flowchart
#   to `skill-map.mmd` (and an SVG if `mmdc` is on your PATH), so the
#   hand-off topology is browsable at a glance.
"""
...
Reads .claude/references/general/skill-graph.md, parses After/Suggest/Reason
triples from markdown tables, categorizes skills by color, and emits a
Mermaid flowchart to .claude/references/general/skill-map.mmd. Optionally runs mmdc for SVG
if available on PATH.

Usage
-----
    python .claude/skills/help/generate_skill_map.py [--svg]
...
SKILL_GRAPH_PATH = REPO_ROOT / ".claude" / "references" / "general" / "skill-graph.md"
OUTPUT_DIR = REPO_ROOT / ".claude" / "references" / "general"
...
def main() -> int:
    ...
    # the .claude/references/general/skill-map.svg render for the working copy.
