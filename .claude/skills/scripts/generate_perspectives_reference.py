#!/usr/bin/env python3
# designer: When you want the public docs to show the catalog of review
#   perspectives -- tag, human name, one-line purpose, optional tier -- I'm
#   the generator that walks `.claude/references/general/review-perspectives/`, reads
#   each file, and writes `seja-public/docs/reference/perspectives.md` as a
#   single alphabetised table so readers can see the full lens set at a glance.
"""
...
Walks `.claude/references/general/review-perspectives/*.md`, extracts the tag
...
Flags:
    --output <path>           Default: <harness-root>/seja-public/docs/
...
# Reuse the frontmatter reader from generate_harness_reference so we
# stay consistent with how the rest of the harness parses YAML blocks.
from generate_harness_reference import (  # noqa: E402
...
def _find_harness_root(start: Path | None = None) -> Path:
...
def collect_perspectives(harness_root: Path, verbose: bool = False) -> list[dict[str, str]]:
    """Return a list of perspective metadata dicts alphabetized by tag."""
    persp_dir = harness_root / ".claude" / "references" / "general" / "review-perspectives"
...
def render_perspectives_reference(
    ...
    if not perspectives:
        return (
            preamble
            + "No review perspectives defined in this harness checkout.\n"
        )

    preamble += (
        f"Generated {generated_at} from `.claude/references/general/review-perspectives/`. "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not harness_root.is_dir():
        print(f"ERROR: harness root does not exist: {harness_root}", file=sys.stderr)
        return 2

    if args.verbose:
        print(f"harness_root: {harness_root}", file=sys.stderr)

    perspectives = collect_perspectives(harness_root, verbose=args.verbose)
    ...
    if output_arg is None:
        output_arg = str(
            harness_root / "seja-public" / "docs" / "reference" / "perspectives.md"
        )
