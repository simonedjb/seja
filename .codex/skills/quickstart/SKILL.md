...
7. **Generate AGENTS.md**: Create a `AGENTS.md` in the codebase root with:
...
13. **Summary**: Output a checklist of everything created, including both the `.codex/` framework files (steps 2–11) and any scaffolding tasks performed (step 12). List remaining manual steps only for tasks the user declined or that failed. Typical remaining manual steps:
...
2. **Run the upgrade script**: Execute `python .codex/skills/scripts/upgrade_framework.py --from <source> --target <target>`. If the user wants a preview first, add `--dry-run`.
