...
7. **Instantiate templates**: Using the questionnaire answers, create project-specific files in `_references/`:
   ...
   - Copy `template/constitution.md` to `project/constitution.md`
   ...
   - **For evolving (brownfield) projects only**: also copy as-is templates
   - **For greenfield projects**: do **not** instantiate as-is files
...
14. **Review & next steps**: Present the generated project specification files and offer review:
    ...
    Then offer: 1) Review specs now, 2) Generate roadmap (`/make-plan --roadmap`), 3) Done for now.

### Mode 2: From Spec File

1. **Locate spec file**: Use the provided path, or look in `specs/`.

2. **Read and parse the spec file**: Same parsing rules as the original quickstart Mode 2.
...
2. **Generate the spec file**: Copy `template/quickstart-spec.md` to `specs/design-spec-YYYY-MM-DD HH.MM UTC.md`.
...
When the user opts into codebase scaffolding, offer the task catalog based on stack choices. See the original quickstart SKILL.md for the full Backend Package Map, Frontend Package Map, and .env Templates.
