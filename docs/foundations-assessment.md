...
| Post-deployment monitoring and "how much of my vision is reflected in actual use" (Barbosa et al. 2021, pp.367-368) | `/explain drift` comparing as-intended and as-coded, plus the drift-detection pipeline | SEJA extends | If drift detection turns out to flag noise more often than meaningful intent-to-code mismatch, the correspondence is operationalized in name but not in function. |
...
| SIM exploring emission of metacommunication (de Souza and Leitao 2009, p.25) | `/critique review` inspecting code and design artifacts from the designer's side | Analogous | If a dedicated segmented-analysis-by-sign-class mode lands in `/critique`, the reading moves toward direct. |
