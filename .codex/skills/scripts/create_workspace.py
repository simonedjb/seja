...
def validate_source(source_root: Path) -> None:
    """Validate that source contains .codex/ and .agent-resources/."""
    if not (source_root / ".codex").is_dir():
...
def run_create(
    ...
    print(f"OK: {prefix}Created _output/ with {len(_OUTPUT_SUBDIRS)} subdirectories and briefs.md")
    ...
    launch_sh_content = (
        '#!/usr/bin/env bash\n'
        '# Launch Codex with the target codebase as an additional directory.\n'
        '# Generated by create_workspace.py -- edit the --add-dir path if the codebase moves.\n'
        'cd "$(dirname "$0")"\n'
        f'codex --add-dir "{target_posix}"\n'
    )
    launch_bat_content = (
        '@echo off\r\n'
        'REM Launch Codex with the target codebase as an additional directory.\r\n'
        'REM Generated by create_workspace.py -- edit the --add-dir path if the codebase moves.\r\n'
        'cd /d "%~dp0"\r\n'
        f'codex --add-dir "{target_win}"\r\n'
    )
