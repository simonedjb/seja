...
- **product-overview.yaml**: Companion data file (`product-overview.yaml`) that captures the product's value chain as a machine-readable graph — layers, personas, quality criteria, and cards with lifecycle status. This file is populated incrementally by `/design` (initial skeleton), `/plan` (new feature cards), `/implement` (status transitions), and `/research` (quality criteria). Running `/document --type spo` generates `_output/docs/spo.html` — an interactive layered diagram — from this file. The `status` field on each card (`proposed`, `implementing`, `done`, `deferred`) is the SPO's version of as-intended / as-coded tracking: a card is `proposed` when designed, `done` when implemented, and the gap between the two is always visible in the diagram. Use `/document --type spo --drift` to highlight that gap.
