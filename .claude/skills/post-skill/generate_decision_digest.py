#!/usr/bin/env python3
# designer: When your project has accumulated design decisions in the Decisions
#   section of product-design-as-intended.md and advisory recommendations
#   across the advisory logs, I'm the digester that harvests both sources and
#   emits `_output/decision-digest.jsonl` -- one JSON line per decision --
#   so tools and reports can query the full decision record without walking
#   every markdown file.
"""
...
Usage
-----
    python .claude/skills/post-skill/generate_decision_digest.py
    python .claude/skills/post-skill/generate_decision_digest.py --verbose

Run from the repository root.
"""

# Rationale for design choices and historical context: see generate_decision_digest-rationale.md in this directory.
...
def scan_advisory_recommendations(verbose: bool = False) -> list[dict]:
    """Scan advisory and research logs for HIGH/MEDIUM recommendations."""
    ...
    for fpath in advisory_files:
        ...
        # Parse header (try Advisory first, then Research)
