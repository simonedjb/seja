...
argument-hint: "<scope> [--plan <id>] [--auto-detect] [--type <readme|contextual-help|api-reference|ddr|help-center|changelog>] [--since <ref>] [--full-history]"
...
| `--type <type>` | No | Force a specific documentation type: readme, contextual-help, api-reference, ddr, help-center, or changelog |
...
2a. **Compute per-doc-type bounding windows** (applies to `--auto-detect` only; skip for other modes):
   ...
   1. Read `${BRIEFS_INDEX_FILE}` (see product-design/conventions.md).
